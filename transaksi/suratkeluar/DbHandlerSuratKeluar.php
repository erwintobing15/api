...
/**
 * All database handler method for transaksi surat keluar data
 * author   : Erwin Tobing
 * date     : 07/01/2021
 */


class DbHandlerSuratKeluar 
{
    ...
    public function allSuratKeluar($id)
    {
        // $sql = "SELECT * FROM tbl_surat_keluar WHERE id_surat!='".$id."' ORDER BY id_surat DESC";
    ...
    public function oneSuratKeluar($id) 
    {
        $sql = "SELECT * FROM tbl_surat_keluar WHERE id_surat = '".$id."'";
        $result = $this->conn->query($sql);
        if ($result->num_rows > 0) {
            header('Content-Type: application/json');
            $data = array();
    ...
    public function insertSuratKeluar($no_agenda,$no_surat,$tujuan,$isi,$tgl_surat,
                                      $tgl_catat,$keterangan,$id_user,$file,$file_temp)
    {
      if ($no_agenda==NULL || $no_surat==NULL || $tujuan==NULL || $isi==NULL  || $keterangan==NULL) {
            ...
            if ($this->conn->query($sql) == TRUE) {
    			move_uploaded_file($file_temp, '../../../upload/surat_keluar/'.$file); //upload file
                header('Content-Type: application/json');
    ...
    public function updateSuratKeluar($id,$no_agenda,$no_surat,$tujuan,$isi,$tgl_surat,$tgl_catat,
                                        $keterangan,$file,$file_temp) 
    {
        $sqlcek = "SELECT * FROM tbl_surat_keluar WHERE id_surat = '".$id."'";
        $result = $this->conn->query($sqlcek); 
        while ($row = $result->fetch_assoc()) {
            if ($row['file'] != NULL) {
                unlink("../../../upload/surat_keluar/".$row['file']); // delete file
            }
        }

        if($file==NULL){
    
             $sql = "UPDATE tbl_surat_keluar 
    ...
    {   
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "DELETE FROM tbl_surat_keluar WHERE id_surat = '".$id."'";
            $this->conn->query($sql);
