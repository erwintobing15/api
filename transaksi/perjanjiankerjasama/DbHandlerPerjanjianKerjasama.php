...
/**
 * All database handler method for transaksi perjanjian kerjasama (PerjanjianKerjasam) data
 * author       : Erwin Tobing
 * created      : 09 Jan 2021 11:35 am
 * modified     : 
 */

class DbHandlerPerjanjianKerjasama
{
    ...
    public function allPerjanjianKerjasama($id)
    {
        $sql = "SELECT * FROM tbl_perjanjian_kerjasama ORDER BY id_surat DESC";
    ...
    public function onePerjanjianKerjasama($id) 
    {
        $sql = "SELECT * FROM tbl_perjanjian_kerjasama WHERE id_surat = '".$id."'";
        $result = $this->conn->query($sql);
        if ($result->num_rows > 0) {
            header('Content-Type: application/json');
            $data = array();
    ...
    public function insertPerjanjianKerjasama($no_agenda,$no_surat,$tujuan,$isi,$tgl_surat,
                                      $tgl_catat,$keterangan,$id_user,$file,$file_temp)
    {
        if ($no_agenda==NULL || $no_surat==NULL || $tujuan==NULL || $isi==NULL  || $keterangan==NULL) {
            ...
            $sql = "INSERT INTO tbl_perjanjian_kerjasama
    ...
    public function updatePerjanjianKerjasama($id,$no_agenda,$no_surat,$tujuan,$isi,
                    $tgl_surat,$tgl_catat,$keterangan,$file,$file_temp) 
    {
        $sqlcek = "SELECT * FROM tbl_perjanjian_kerjasama WHERE id_surat = '".$id."'";
        ...
        if($file==NULL){
             ...
             $sql = "UPDATE tbl_perjanjian_kerjasama 
    ...
    public function deletePerjanjianKerjasama($id)
    {   
        $sqlcek = "SELECT * FROM tbl_perjanjian_kerjasama WHERE id_surat = '".$id."'";
        $result = $this->conn->query($sqlcek);    
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "DELETE FROM tbl_perjanjian_kerjasama WHERE id_surat = '".$id."'";
            $this->conn->query($sql);
