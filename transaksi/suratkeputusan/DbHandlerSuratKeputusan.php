...
/**
 * All database handler method for transaksi surat keputusan (SuratKeputusan) data
 * author       : Erwin Tobing
 * created      : 09 Jan 2021 08:59 am
 * modified     : 
 */

class DbHandlerSuratKeputusan
{
    ...
    public function allSuratKeputusan($id)
    {
        $sql = "SELECT * FROM tbl_surat_keputusan ORDER BY id_surat DESC";
    ...
    public function oneSuratKeputusan($id) 
    {
        $sql = "SELECT * FROM tbl_surat_keputusan WHERE id_surat = '".$id."'";
        $result = $this->conn->query($sql);
        if ($result->num_rows > 0) {
            header('Content-Type: application/json');
            $data = array();
    ...
    public function insertSuratKeputusan($no_agenda,$no_surat,$tujuan,$isi,$tgl_surat,
                                      $tgl_catat,$keterangan,$id_user,$file,$file_temp)
    {
        if ($no_agenda==NULL || $no_surat==NULL || $tujuan==NULL || $isi==NULL  || $keterangan==NULL) {
            ...
            $sql = "INSERT INTO tbl_surat_keputusan
    ...
    public function updateSuratKeputusan($id,$no_agenda,$no_surat,$tujuan,$isi,
                    $tgl_surat,$tgl_catat,$keterangan,$file,$file_temp) 
    {
        $sqlcek = "SELECT * FROM tbl_surat_keputusan WHERE id_surat = '".$id."'";
        ...
        if($file==NULL){
             ...
             $sql = "UPDATE tbl_surat_keputusan 
    ...
    public function deleteSuratKeputusan($id)
    {   
        $sqlcek = "SELECT * FROM tbl_surat_keputusan WHERE id_surat = '".$id."'";
        $result = $this->conn->query($sqlcek);    
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "DELETE FROM tbl_surat_keputusan WHERE id_surat = '".$id."'";
            $this->conn->query($sql);
