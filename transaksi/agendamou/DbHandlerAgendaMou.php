...
/**
 * All database handler method for agenda mou (AgendaMou) data
 * author       : Erwin Tobing
 * created      : 09 Jan 2021 14:12 pm
 * modified     : 
 */

class DbHandlerAgendaMou
{
    ...
    public function allAgendaMou($id)
    {
        $sql = "SELECT * FROM tbl_agenda_mou ORDER BY id_surat DESC";
    ...
    public function oneAgendaMou($id) 
    {
        $sql = "SELECT * FROM tbl_agenda_mou WHERE id_surat = '".$id."'";
        $result = $this->conn->query($sql);
        if ($result->num_rows > 0) {
            header('Content-Type: application/json');
            $data = array();
    ...
    public function insertAgendaMou($no_agenda,$no_surat,$tujuan,$isi,$tgl_surat,
                                      $tgl_catat,$keterangan,$id_user,$file,$file_temp)
    {
        if ($no_agenda==NULL || $no_surat==NULL || $tujuan==NULL || $isi==NULL  || $keterangan==NULL) {
            ...
            $sql = "INSERT INTO tbl_agenda_mou
    ...
    public function updateAgendaMou($id,$no_agenda,$no_surat,$tujuan,$isi,
                    $tgl_surat,$tgl_catat,$keterangan,$file,$file_temp) 
    {
        $sqlcek = "SELECT * FROM tbl_agenda_mou WHERE id_surat = '".$id."'";
    ...
    public function deleteAgendaMou($id)
    {   
        $sqlcek = "SELECT * FROM tbl_agenda_mou WHERE id_surat = '".$id."'";
        $result = $this->conn->query($sqlcek);    
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "DELETE FROM tbl_agenda_mou WHERE id_surat = '".$id."'";
            $this->conn->query($sql);
